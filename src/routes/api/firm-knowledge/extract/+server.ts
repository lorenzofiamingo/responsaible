...
import { extractFirmKnowledge } from '$lib/server/extract/knowledge';
import { pdfToText } from '$lib/server/extract/pdf';
import { json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';

// Same ceilings as the work-product intake: a PDF is fully buffered and pdf.js's
// object graph is several× the raw size, so a large file risks the workerd cap.
const MAX_BYTES = 8 * 1024 * 1024; // 8 MB upload ceiling
const MAX_CHARS = 60_000; // extraction input ceiling
const MIN_CHARS = 12; // below this there's nothing to extract

/**
 * Firm-knowledge intake. Accepts either a multipart upload (`file`: PDF/Word/text)
 * or a JSON body (`{ text, filename }`), extracts the text, and returns a
 * REVIEW-READY knowledge draft (NOT persisted). The /knowledge/new page binds the
 * result so a supervisor can correct it before it enters the shared corpus.
 */
export const POST: RequestHandler = async ({ request, platform, locals }) => {
	if (!locals.user || !CAN_SUBMIT.has(locals.user.role)) {
		return json({ error: 'Only a supervising lawyer can ingest firm knowledge.' }, { status: 403 });
	}

	// Reject oversized uploads from Content-Length BEFORE buffering the body.
	...
	const draft = await extractFirmKnowledge(text, { sourceKind, filename, apiKey });

	if (overflow > 0) {
		draft.meta.warnings.push(
			`Document exceeds the ${MAX_CHARS.toLocaleString('en-GB')}-character limit; the last ${overflow.toLocaleString('en-GB')} characters were not analysed.`
		);
	}
